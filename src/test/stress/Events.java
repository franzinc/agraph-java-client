...
public class Events {
	...
	private static class Defaults {
	    ...
	    static boolean DELETE_WORKERS = true;
		...
		public static void init(String[] args) throws Exception {
			...
			options.addOption(OptionBuilder.withLongOpt("delete")
					...
					.withDescription("use 2 deleting processes [default=" + DELETE_WORKERS + "]")
					.create("d"));
			...
			if (cmd.hasOption("help")) {
				...
				formatter.printHelp("./events [options]",
						...
						"./events.sh --status 500 -v 1 -l 1 -d false -q 1 -m 1 --size 100000 --sparql\n" +
						"# Runs less verbose status, single-threaded, creating a small repo, no deletes, sparql queries.\n" +
						"./events.sh --supersede -l 10 -q 2 -m 2 --size 1m\n" +
			...
			if (cmd.hasOption("seed")) {
				RANDOM.setSeed(Long.parseLong(cmd.getOptionValue("seed")));
			}
		}
...
//						Stmt.statementSet(tupleQuery2.evaluate()));
	...
	public static void main(String[] args) throws Exception {
	    ...
	    /////////////////////////////////////////////////////////////////////// PHASE 5
	    if (Defaults.PHASE <= 5 && Defaults.DELETE_WORKERS) {
	    ...
	    if (Defaults.PHASE <= 6 && Defaults.MIXED_RUNS != 0) {
	    	...
	    	for (int run=0; run < Defaults.MIXED_RUNS || Defaults.MIXED_RUNS == -1; run++) {
		    	...
		    	trace("%d total triples returned over %d queries in " +
	        	"%.1f seconds (%.2f triples/second, %.2f queries/second, " +
	        	"%d triples/query, %d triples added, %d deletes).", triples, queries, seconds, triples/seconds,
	        	queries/seconds, (queries==0 ? 0 : triples/queries), added, deleted);
	    	}
	    	executor.shutdown();
	    	Monitor.stop();
	    }
