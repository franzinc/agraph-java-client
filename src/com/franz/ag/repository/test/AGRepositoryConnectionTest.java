...
public class AGRepositoryConnectionTest extends RepositoryConnectionTest {
	...
	public void testSetWithVariables() throws Exception {
		...
		sq.setFrom(new String[]{context1.toString()});
		sq.setFromNamed(new String[]{context2.toString()});
		sq.setWithVariables(ts, new Object[]{"name",ts.createLiteral("Bob")});
		assertFalse(sq.ask());
		
		sq.setFromNamed(new String[]{context1.toString()}); 
		assertTrue(sq.ask());
	}
	...
	public void testGetStatements() throws Exception {
		...
		try {
			...
			while (result.hasNext()) {
				Statement st = result.next();
				assertNull("Statement should not be in a context ", st.getContext());  // FIXME: bug18178
	...
	@Ignore public void testStatementSerialization() throws Exception {
		testCon.add(bob, name, nameBob);
